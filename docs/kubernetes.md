...
Локальный профиль и точные команды запуска в kind описаны в [deploy/k8s/overlays/local/README.md](../deploy/k8s/overlays/local/README.md). 23 сентября 2026 года он был проверен на одном узле: 3/3 API, 2/2 CPU NER и 3/3 Redis-пода стали Ready; `/process` маскировал и точно восстановил строку. После удаления `redis-0` Sentinel избрал `redis-1`, прежний primary присоединился как replica, а созданное до переключения соответствие восстановилось. Это проверка локального failover пода, а не отказоустойчивости при потере узла или производительности GPU-профиля.
