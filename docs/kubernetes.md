...
Текущий локальный GPU-профиль и команды запуска в kind описаны в [deploy/k8s/overlays/local-gpu/README.md](../deploy/k8s/overlays/local-gpu/README.md). 24 сентября 2026 года он был проверен на одном узле: 3/3 API, 2/2 GPU RuBERT и 3/3 Redis-пода стали Ready; публичный `/process` маскировал и точно восстановил строку. Это не подтверждает устойчивость к потере узла или 2000 RPS на новом сетевом пути.

[Отдельный нагрузочный тест](kind-gpu-rps-20260924.md) подтвердил 2000 RPS на
локальном входе в kind, но показал ошибки при такой нагрузке через публичный
Cloud.ru/SSH-шлюз.

Предыдущий [CPU-профиль](../deploy/k8s/overlays/local/README.md) был проверен 23 сентября 2026 года: 3/3 API, 2/2 CPU NER и 3/3 Redis-пода стали Ready. После удаления `redis-0` Sentinel избрал `redis-1`, прежний primary присоединился как replica, а созданное до переключения соответствие восстановилось.
