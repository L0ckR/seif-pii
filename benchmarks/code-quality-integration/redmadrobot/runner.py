...
def main():
    ...
    actual_masked_alnum_offsets = {mode: {} for mode in predictions}
    gold_alnum_offsets = {}
    raw_predictions = {mode: {} for mode in predictions}
    masked_text_sha256 = {mode: {} for mode in predictions}
    for row in rows:
        ...
        gold_alnum_offsets[key] = sorted({i for kind, start, end in truth[key] if kind in COMMON | STRUCTURED
                                          for i in range(start, end) if text[i].isalnum()})
        for mode, spans in outputs.items():
            raw_predictions[mode][key] = [[s.type, s.start, s.end] for s in spans]
            masked, replacements = mask(text, spans, 'mask')
            masked_text_sha256[mode][key] = hashlib.sha256(masked.encode()).hexdigest()
            assert len(masked) == len(text)
            assert restore_exact({'masked': masked, 'replacements': replacements}) == text
            actual_masked_alnum_offsets[mode][key] = [i for i, char in enumerate(text)
                                                    if char.isalnum() and masked[i] == '*']
    ...
    result = {'revision': args.revision, 'measured_at_utc': datetime.now(timezone.utc).isoformat(),
              ...
              'actual_masked_alnum_offsets': actual_masked_alnum_offsets,
              'raw_predictions': raw_predictions,
              'masked_text_sha256': masked_text_sha256,
              'gold_alnum_offsets': gold_alnum_offsets,
              'paired_mapped_spans': {
                  'gold': {key: sorted(span for span in spans if span[0] in COMMON | STRUCTURED) for key, spans in truth.items()},
                  **{mode: {key: sorted(spans) for key, spans in rows.items()} for mode, rows in predictions.items()},
              },
              'validation': {'source_and_inputs_immutable': True, 'cached_gold_matches_csv': True,
                             ...
                             'actual_mask_and_restore_checked_all_modes_all_rows': True,
