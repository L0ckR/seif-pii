...
def _start_topology(tmp_path, executable, ports, password):
    processes = []
...
async def _until(check, timeout=20):
...
async def _replicated_and_discovered(vault, replicas, key, ports):
...
def _write_failover_report(ports, new_master, recovered_seconds):
    primary_port = ports[0]
    report = {
...
async def _verify_sentinel_failover(ports, password, processes):
    import httpx
    ...
    try:
        await _until(lambda: vault.redis.ping())
        async with (
            ...
            await _until(lambda: _replicated_and_discovered(vault, replicas, key, ports))
            ...
            new_master = await _until(promoted)
            await _until(lambda: vault.get(key))
            ...
            _write_failover_report(ports, new_master, recovered_seconds)
    finally:
        await vault.close()
        await asyncio.gather(*(replica.aclose() for replica in replicas))


def _stop_topology(processes):
...
def test_real_sentinel_failover_retains_replicated_ciphertext(tmp_path):
    ...
    processes = _start_topology(tmp_path, executable, ports, password)
    try:
        asyncio.run(_verify_sentinel_failover(ports, password, processes))
    finally:
        _stop_topology(processes)
