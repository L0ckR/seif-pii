...
FILES = {
    "data/test-00000-of-00001.parquet": (
        "scanpatch-test.parquet",
        "5d44f419692282285ae811bd54b69e57bb44121e31f3628408e203f9416371ba",
    ),
...
GOLD_MAP = {
    **dict.fromkeys(PERSON_LABELS, "PERSON"),
    **dict.fromkeys(ADDRESS_LABELS, "LOCATION"),
    "email": "EMAIL",
    "mobile_phone": "PHONE",
    "tin": "INN",
...
OTHER_LABELS = frozenset(
    ("nickname", "ip", "snils", "vehicle_number", "military_individual_number", "organization", "date")
)
...
def _validate_row(row):
    ...
    for start, end, kind, value in zip(*fields, strict=True):
        if (
            kind not in set(GOLD_MAP) | OTHER_LABELS
...
def _infer_rows(rows, analyzer, raw_path, truth):
    ...
    with raw_path.open("w", encoding="utf-8") as stream:
        for row in rows:
            ...
            candidates = [
                Span(item.start, item.end, item.entity_type, item.score, "ner")
                for item in upstream
                if item.entity_type in {"PERSON", "LOCATION"}
            ]
                ...
                json.dumps(
                    {
                        ...
                        "merged_predictions": {system: sorted(values[key]) for system, values in predictions.items()},
                    },
                    ensure_ascii=False,
                )
                + "\n"
...
def main():
    ...
    truth = {
        row["id"]: merge_adjacent(row["text"], coarsen(row["fine_gold"], GOLD_MAP, keep_unknown=True)) for row in rows
    }
    ids = [key for key, spans in truth.items() if all(kind in COMMON for kind, *_ in spans)]
    metadata = {
        "repository": REPOSITORY,
        "revision": REVISION,
        "license_in_pinned_card": "MIT",
        "card_url": f"https://huggingface.co/datasets/{REPOSITORY}/blob/{REVISION}/README.md",
        "original_splits": {"train": 4786, "test": 532},
        "evaluated_split": "entire test, no train download",
        ...
        "gold_empty_rows": sum(not row["fine_gold"] for row in rows),
        "common_whole_cases": len(ids),
        "common_case_ids": ids,
    ...
    protocol = {
        "dataset": metadata,
        "mapping": {"gold": GOLD_MAP, "seif": LOCAL_MAP, "presidio": PRESIDIO_MAP},
        ...
        "pdf_overlap": {
            "names": sorted(PERSON_LABELS),
            "contacts": ["email", "mobile_phone"],
            "tax_identifier": ["tin"],
            "7_address_labels_with_direct_counterpart": [
                "address",
                "address_country",
                "address_city",
                "address_street",
            ...
            json.dumps(
                {
                    ...
                    "negatives": metadata["gold_empty_rows"],
    ...
    report = {
        ...
        "evaluation_status": "first frozen-implementation evaluation of this previously unused mixed-language test"
        if first
        ...
        "common_schema_primary": primary,
        "character_uncertainty": paired_bootstrap(
            typed_characters(selected_gold),
            {system: typed_characters(values) for system, values in selected_pred.items()},
            {"all": ids},
            COMMON,
        ),
