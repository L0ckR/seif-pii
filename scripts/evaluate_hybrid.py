...
def main():
    ...
    for name, corpus in baseline["corpora"].items():
        ...
        for row in corpus["raw_cases"]:
            ...
            base = detect(text)
            candidates = [Span(item["start"], item["end"], "PERSON", item["score"], "cached-presidio")
                          for item in row["presidio"] if item["type"] == "PERSON"]
            ...
            rows.append({"case_id": case_id, "expected": sorted(truth[case_id]),
                         "current_fast": sorted(current_fast[case_id]), "hybrid": sorted(hybrid[case_id]),
                         "original_fast": row["seif"], "cached_ner_candidates": len(candidates)})
        ...
        corpora[name] = {
            "case_count": corpus["case_count"], "canonical_corpus_sha256": corpus["canonical_corpus_sha256"],
            "common_case_ids": common_ids, "common_strict4": common_results,
            "fine_taxonomy_requirement_fit": {"current_fast": metric(truth, current_fast), "hybrid": metric(truth, hybrid)},
            "original_presidio_common_strict4": corpus["common_strict4"]["presidio"], "raw_cases": rows,
        }
    ...
    print(json.dumps({name: {system: {key: values[key] for key in ("precision", "recall", "f1")}
                             for system, values in corpus["common_strict4"].items()}
                      for name, corpus in corpora.items()}, indent=2))
