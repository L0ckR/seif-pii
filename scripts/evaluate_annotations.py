...
def positions(text, entities, *, typed=False):
    return {
        (entity["type"], index) if typed else index
...
def summarize(counts):
    ...
    result["fully_protected_positive_rate"] = (
        round(counts["fully_protected_positive_cases"] / counts["positive_cases"], 6)
        if counts["positive_cases"]
        else None
    )
    result["false_positive_negative_rate"] = (
        round(counts["false_positive_negative_cases"] / counts["negative_cases"], 6)
...
def _update_type_coverage(text, gold, actual, type_counts):
    ...
    for kind in {s["type"] for s in gold}:
        ...
        type_counts[kind].update(
            gold_characters=len(relevant),
            protected_characters=len(relevant & actual),
            cases=1,
            fully_protected_cases=int(relevant <= actual),
        )
        if relevant - actual:
            missing.append(kind)
    return missing
...
def evaluate(inputs, annotations, predictions, weights=None):
    ...
    for case_id, source in inputs.items():
        ...
        counts = {
            ...
            "cases": 1,
            "exact_cases": int(expected == actual),
        ...
        typed_totals.update(
            tp=len(gold_typed & pred_typed), fp=len(pred_typed - gold_typed), fn=len(gold_typed - pred_typed)
        )
        gold_spans = {(s["type"], s["start"], s["end"]) for s in gold}
        pred_spans = {(s["type"], s["start"], s["end"]) for s in guessed}
        exact_span.update(
            tp=len(gold_spans & pred_spans), fp=len(pred_spans - gold_spans), fn=len(gold_spans - pred_spans)
        )
        ...
        diagnostics.append(
            {
                "case_id": case_id,
                **counts,
                "traffic_weight": weights[case_id],
                "gold_types": sorted({s["type"] for s in gold}),
                "missed_types": sorted(missing),
            }
        )

    return {
        "unique_case_primary": summarize(totals),
        "traffic_weighted_secondary": summarize(weighted),
        ...
        "gold_type_coverage": {
            kind: {
                **counts,
                "character_recall": round(counts["protected_characters"] / counts["gold_characters"], 6)
                if counts["gold_characters"]
                else None,
            }
        ...
        json.dumps(
            {
                ...
                for key in (
                    "unique_case_primary",
                    "traffic_weighted_secondary",
                    "certain_cases_sensitivity",
                    "gold_type_coverage",
                )
            },
            ensure_ascii=False,
