...
def infer_corpus(model, cases: dict, args, torch) -> tuple[list[dict], dict]:
    ...
    for index, (case_id, row) in enumerate(cases.items(), 1):
        ...
        rows.append(
            {"case_id": case_id, "text_sha256": hashlib.sha256(text.encode()).hexdigest(), "entities": entities}
        )
        if index % 50 == 0:
    ...
    return rows, {
        ...
        "sequential_documents_per_second": len(cases) / elapsed,
        "latency_ms": {
...
def generate(args) -> dict:
    ...
    model = AutoExtractor.from_pretrained(
        str(args.model_path), map_location=args.device, quantize=args.fp16, compile=False, local_files_only=True
    )
    tokenizer = model.processor.tokenizer
    markers = [
        item
        for item in json.loads((args.model_path / "tokenizer.json").read_text())["added_tokens"]
        if item["id"] >= 250102
    ]
    if len(tokenizer) != 250112 or any(
        tokenizer.convert_tokens_to_ids(item["content"]) != item["id"] for item in markers
    ):
    ...
    metadata = {
        ...
        "model_file_sha256": fingerprint_model(args.model_path),
        "cases": len(cases),
        "candidates": sum(len(row["entities"]) for row in rows),
        "versions": {
            name: importlib.metadata.version(name)
            for name in ("gliner2", "torch", "transformers", "tokenizers", "protobuf", "sentencepiece")
        },
        "configuration": {
            "schema": args.schema,
            "labels": SCHEMAS[args.schema],
            "threshold": args.threshold,
            "overlap_policy": "flat",
            "max_len": None,
            "batch_size": 1,
        ...
        "contract": {
            "candidates_over_200_characters": sum(e["end"] - e["start"] > 200 for row in rows for e in row["entities"])
        },
        ...
        json.dumps(
            {
                ...
                "performance": {k: v for k, v in metadata["performance"].items() if k != "per_case_latency_ms"},
            },
            indent=2,
        )
    )
