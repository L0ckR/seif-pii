...
def check_replica(client):
    ...
    if (
        state.get("role") != "slave"
        or state.get("master_link_status") != "up"
...
class _CaptureSession:
    ...
    def run(self):
        ...
        print(
            json.dumps(
                {"capture_directory": str(self.directory), "stop_reason": self.stop_reason, "counts": dict(self.stats)},
                ensure_ascii=False,
            ),
            flush=True,
        )
