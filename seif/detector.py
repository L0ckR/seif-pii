...
def _is_public_name(text: str, start: int, end: int) -> bool:
    before = text[max(0, start - 65) : start]
    ...
    if _PERSONAL_NAME.search(before):
        return False
    if _PUBLIC_NAME.search(before) or _HISTORICAL.fullmatch(value):
        return True
    ...
    if following:
        return bool(
            _HISTORICAL.fullmatch(value + following.group()) or _PUBLIC_NAME_APPOSITION.match(after[following.end() :])
        )
    return False
...
def _resolve(candidates: Iterable[Span]) -> list[Span]:
    ...
    def flush() -> None:
        ...
        starts: list[int] = []
        for span in sorted(
            cluster, key=lambda s: (-_PRIORITY.get(s.type, 85), -(s.end - s.start), -s.confidence, s.start)
        ):
...
def detect(text: str, *, extra_rules: list[dict] | None = None) -> list[Span]:
    ...
    if any(c.isdigit() for c in text):
        add(_PHONE, "PHONE", "russian-phone-format", 0.98)
        if "+" in text:
            add(
                _INT_PHONE,
                "PHONE",
                ...
                0.98,
                lambda value, *_: 8 <= len(_digits(value)) <= 15,
            )
        ...
        add(_INN_BARE, "INN", "checksum", 0.94, lambda v, s, e: _valid_inn(v) and not _is_public_inn(text, s, v))
        add(_CARD_LABEL, "CARD", "explicit-card-context", 0.98)
        add(_CARD_BARE, "CARD", "luhn-checksum", 0.96, lambda v, *_: _luhn(v))
        add(_CVV, "CVV")
        add(_PIN, "PIN")
        ...
        if "выда" in lower:
            add(
                ...
                "PASSPORT_DATE",
                check=lambda v, s, e: (
                    _valid_date(v) and bool(re.search(r"паспорт|дата[ \t]+выдачи", text[max(0, s - 120) : s], _FLAGS))
                ),
            )
            if "паспорт" in lower:
                add(_PASS_DATE_LATE, "PASSPORT_DATE", check=lambda v, *_: _valid_date(v))
        add(_POSTAL, "POSTAL_CODE", check=lambda _, s, e: not _is_public_address(text, s))
        add(_ADDRESS_POSTAL, "POSTAL_CODE", "address-format", 0.95, lambda _, s, e: not _is_public_address(text, s))
    ...
    if any(label in lower for label in ("выдан", "орган", "мвд", "увд", "фмс")):
        ...
        if "паспорт" in lower:
            add(_ISSUER_DIRECT, "PASSPORT_ISSUER", confidence=0.95)
