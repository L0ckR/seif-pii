...
_NOT_A_NAME = frozenset(
    ...
    "ожидает подтвердил подтверждает обратился позвонил ответил отказался заполняет "
    "согласился проверяет сообщил подписывает прочитал получил отправил "
    "вчера сегодня завтра звонка заявку заявление данные согласие отказ "
    ...
    "received requested approved processing executed rejected declined company corporate business bank ltd llc inc "
    "called returned arrived departed checked signed supplied provided spoke reports writes "
    "yesterday today tomorrow waiting contacted submitted verified".split()
)
